<?php
/*
Plugin Name: Google Maps Short Code
Plugin URI: http://www.dkarfawork.com/wp/google-maps-code
Contributors: Debabrata Karfa
Description: Adds a Google Maps short code for Satelite view
Version: 1.0.0
...
function dkgm_map_shortcode( $atts ) {
...
add_shortcode( 'dkgm_map', 'dkgm_map_shortcode' );
...
function dkgm_map_load_scripts() {
	wp_register_script( 'google-maps-api', 'http://maps.google.com/maps/api/js?sensor=false' );
}
add_action( 'wp_enqueue_scripts', 'dkgm_map_load_scripts' );
...
function dkgm_map_get_coordinates( $address, $force_refresh = false ) {
...
add_action( 'wp_head', 'dkgm_map_css' );